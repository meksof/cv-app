<template>
  <div class="mt-4">
    ...
    <!-- Projects cards -->
    <div
      class="card projects mt-3"
      :hidden="
        appliedFilter !== undefined &&
          appliedFilter.length > 1 &&
          !cardHasKeyword(project, appliedFilter)
      "
      v-bind:class="{
        highlighted: appliedFilter !== undefined && appliedFilter.length > 1
      }"
    ...
    >
      <div class="card-header row">
        <div class="col-5 col-sm-5">
          <h3
            class="card-title"
            v-html="highlight(project.title, appliedFilter)"
          ></h3>
          <span
            class="card-subtitle no-wrap"
            v-html="highlight(project.client, appliedFilter)"
          ></span>
        </div>
        <div
          v-if="project.duration"
          class="col-3 col-sm-3 d-flex align-items-start"
        >
        ...
        <div
          v-if="project.team"
          class="col-4 col-sm-4 d-flex align-items-start"
        >
      ...
      <div class="card-content mt-2">
        <div
          class="description"
          v-html="highlight(project.description, appliedFilter)"
        ></div>
        <div class="more-info mt-2">
          <div
            v-if="
              project.roleList && Object.keys(project.roleList).length !== 0
            "
            class="more-info-trigger"
            v-bind:class="{
              open:
                project.moreInfo || (appliedFilter && appliedFilter.length > 1)
            }"
          ...
          <ul
            class="role"
            v-if="project.roleList && project.roleList.length != 0"
          >
            <li
